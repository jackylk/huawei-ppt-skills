---
name: global-tech-strategy-ppt
description: "Use when the user asks for global technology strategy consulting-style Chinese technical planning PPTs, insight-to-plan narratives, high-density enterprise strategy slides, architecture planning pages, or technical roadmap decks such as Agentic AI/data infrastructure planning."
---

# Global Tech Strategy PPT

Use this skill for Chinese enterprise technical-planning decks where the job is to turn dense insight material into a sharp technical plan with a global technology strategy consulting visual style.
...
Keep the narrative and writing discipline of a serious technical planning deck, but use a global technology strategy consulting style instead of a single-brand red/white corporate style.

Reference feel:

- McKinsey/BCG/Bain strategy report discipline: structured, evidence-led, executive-readable, chart-first.
- a16z/State of AI/CB Insights market-map sharpness: category maps, value migration, startup landscape, investment thesis.
- Databricks/Snowflake/NVIDIA-style technical strategy clarity: architecture diagrams, capability stacks, roadmap gates, technical mechanism pages.

Visual principles:

- **Background:** white, off-white, or very light cool gray. Avoid heavy black backgrounds and decorative gradients.
- **Palette:** charcoal `#111827`, slate `#374151`, cool gray `#E5E7EB`, deep blue `#1D4ED8`, teal `#0F766E`, amber `#B45309`, muted crimson `#B91C1C`.
- **Accent use:** blue/teal for strategy direction and architecture flow, amber for market timing or risk, muted crimson only for contradiction, gap, or final judgment. Accent area should stay restrained.
- **Typography:** crisp consulting-report hierarchy: bold claim title, compact subtitle, small dense labels, aligned tables, concise evidence notes.
- **Layout:** high-density but calm; prefer grids, matrices, architecture stacks, value-chain maps, market maps, roadmap swimlanes, decision trees, and mechanism diagrams.
- **No brand imitation:** no corporate logo imitation, no famous consulting-firm logo, no fake vendor UI, no large red blocks, no neon cyberpunk style.
- **Chapter tag:** small top-right gray or blue-gray pill, not a red label.
...
The user may rename the parts, but do not merge them. Technical strategy planning decks need evidence-led insight pages before architecture planning pages.
...
### Stage 6: Slide Plan
   ...
   - For each slide, write title, claim, proof object, layout, accent judgment, and key source needs.
...
For a request like "给 Agent DB 做一个全球科技战略咨询风技术规划 PPT", respond with a scoped planning question, not a draft deck:
...
- charcoal judgment title with optional muted accent keyword
...
- restrained consulting-style conclusion treatment: thin blue/teal rule, muted crimson gap marker, pale callout, or small judgment label
...
Visual density: high-density global technology strategy consulting page; avoid sparse layout, avoid oversized empty cards, use 6-12 content units
Text discipline and accent discipline
```
...
```text
Layout: left 62% eight-layer architecture stack, right 32% key object cards, bottom restrained consulting-style viewpoint.
Architecture layers from bottom to top: 多端输入采集、事件标准化、上下文快照、意图包解析、权限票据、Agent 路由、执行回执、个性化记忆.
Show deep-blue arrows from 意图包 -> 权限票据 -> Agent 路由 -> 执行回执, and a muted-crimson marker only on the risk/gap node.
...
Bottom viewpoint: one strategic judgment or threat/opportunity, preferably as a pale-blue/teal callout or muted-crimson gap note
...
Keep the global technology strategy consulting style restrained. Use color as a precision signal rather than decoration.

- Prefer **deep blue** for strategy direction, **teal** for architecture flow or operating model, **amber** for timing/risk, and **muted crimson** only for contradiction, gap, threat, or final judgment.
- Avoid large colored frames, thick borders, full-color panels, repeated rectangles, decorative blobs, and dramatic gradients. They reduce the serious strategy-report feel.
- Keep all accent colors together roughly under 8-12% of a dense planning slide. Most structure should be white, light gray, dark gray, and black.
- Replace big colored boxes with one of these treatments:
  - 2px blue/teal rule plus black judgment text.
  - Pale blue/teal/amber callout background with a 1-2px border.
  - White callout with a 4-6px left bar in blue, teal, amber, or muted crimson.
  - Small label such as `Judgment` / `Implication` / `Risk` / `Gap` followed by black body text.
- Bottom viewpoints should not default to heavy outlined boxes. Use a restrained callout unless the slide needs an exceptional warning.
- Chapter tags should be small gray, blue-gray, or pale-blue pills; do not use solid red tags.
...
Accent emphasis: <one phrase or path>
...
Style: global technology strategy consulting report, white or very light cool-gray background, charcoal title, dark gray subtitle, dense but clean consulting layout, thin gray frames, deep blue/teal/amber accents, muted crimson only for contradiction or final judgment, no logo, no watermark, no black panels, no gradient glow.
...
Bottom viewpoint treatment: restrained consulting-style callout, preferably pale blue/teal/amber background with thin border or a left accent bar; use muted crimson only for explicit gap/threat judgment.
Visual density: high-density global technology strategy consulting page, 6-12 meaningful content units, avoid sparse layout and oversized empty cards.
...
```bash
python ~/.codex/skills/global-tech-strategy-ppt/scripts/make_image_pptx.py \
...
- Normal content boxes use gray outlines; blue/teal are reserved for architecture flow and strategy direction; amber for timing/risk; muted crimson for contradiction, gap, or threat.
- Accent colors must feel like consulting-report signals, not decoration: avoid thick colored boxes, solid panels, and overusing one accent on every container.
